...
Перевір чи видалив з аркушу Analyzer-МВЗ в файлі ecom_tables.xlsx статті МаркетКур'єрсДостав та ДохідПослКінцевСпож 
і чи додав ти ці статті в аркуш 100 цього ж файлу

Стаття Недоотримана виручка має бути і в аркуші Analyzer-МВЗ, і в аркуші 100

Крім того, стаття ФОП збірники має бути з від'ємними значеннями

Також треба слідкувати за актуальністю довідників:
business_analytic.ecom.channel_vytraty, [Business_Analytic].[ecom].[dovidnyk_statti] та [Business_Analytic].[ecom].[dovidnyk] - 
Якщо якась стаття змінила своє джерело - то обов'язково треба це змінити в цих таблицях.

! Важливо - якщо треба змінити/перерахувати попередні місяці - треба слідкувати щоб дані в цих словниках були актуальні на відповідний місяць!
...
SELECT [year]
  ...
  where month=@month and year=@year and type_vytraty=70300900

  insert into [Business_Analytic].[ecom].[analyzer_mvz]
  select year, month, mvz, [fil.Filials.filialNameUA], 70300900, 'ДохідПослКінцевСпож', sum([Грн_без_пдв])
  from #to_ecom_t
  where month=@month and year=@year and [Показник]='ДохідПослКінцевСпож_без_пдв'
  group by year, month, mvz, [fil.Filials.filialNameUA]
...
delete from #tmp_f
where [Міс.вин.п.] in ('20211A2527', '20211I0218', '20211RC014', '20211RC016', '20200A6000', '20210A2529', '20211I0220')
...
left join business_analytic.ecom.vyruchka_final v on a.mvz=v.mvz and a.year=v.year and a.month=v.month
left join business_analytic.ecom.sap_info s on a.year=s.year and a.month=s.month and a.type_vytraty=s.type_vytraty
where a.type_vytraty=719007 and a.month=@month and a.year=@year
	...
	and s.type_vytraty=811000; -- !!!!!!!!!!!!!!!!! правка 13.03.26 (для січня 2026)
...
delete from [Business_Analytic].[ecom].[analyzer_mvz]
where name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') and month=@month

insert into [Business_Analytic].[ecom].[analyzer_mvz]
select * from [Business_Analytic].[ecom].[analyzer_mvz_bonuses_100]
where year=@year and month=@month and name_vytraty<>'ДохідПослКінцевСпож'
...
select a.*,
0 [Glovo],
case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') 
	then 
		(case when isnull(v.[Glovo], 0)=isnull(v.[iPost] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)=0 then 0 
				else  isnull(v.[iPost], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) 
	when name_vytraty='ДохідПослКінцевСпож'
		then (case when isnull(v.[Glovo], 0)=isnull(v.[iPost] , 0) or isnull(v.[Самовывоз shop.fora.ua], 0)=isnull(v.[iPost] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0)=0 then 0 
				else  isnull(v.[iPost], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0))*a.sum_vytraty end) end) 
	end [iPost],
...
case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') 
	then 
		(case when isnull(v.[Glovo], 0)=isnull(v.[Uklon] , 0) then 0 
				...
				else  isnull(v.[Uklon], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) 
	when name_vytraty='ДохідПослКінцевСпож'
		then (case when isnull(v.[Glovo], 0)=isnull(v.[Uklon] , 0) or isnull(v.[Самовывоз shop.fora.ua], 0)=isnull(v.[Uklon] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0)=0 then 0 
				else  isnull(v.[Uklon], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0))*a.sum_vytraty end) end) 
	end [Uklon],

case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') 
	then (case when isnull(v.[Glovo], 0)=isnull(v.[Доставка shop.fora.ua] , 0) then 0 
		else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)=0 then 0 
			else  isnull(v.[Доставка shop.fora.ua], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) 
	when name_vytraty='ДохідПослКінцевСпож'
		then (case when isnull(v.[Glovo], 0)=isnull(v.[Доставка shop.fora.ua] , 0) or isnull(v.[Самовывоз shop.fora.ua], 0)=isnull(v.[Доставка shop.fora.ua] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0)=0 then 0 
				else  isnull(v.[Доставка shop.fora.ua], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0))*a.sum_vytraty end) end)
	end [Доставка shop.fora.ua],

case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка')  then 
	(case when isnull(v.[Glovo], 0)=isnull(v.[Самовывоз shop.fora.ua] , 0) then 0 
	else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)=0 then 0 
		else  isnull(v.[Самовывоз shop.fora.ua], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) else 0 end [Самовывоз shop.fora.ua],

case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') 
	then (case when isnull(v.[Glovo], 0)=isnull(v.[Рой] , 0) then 0 
		else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)=0 then 0 
			else  isnull(v.[Рой], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) 
	when name_vytraty='ДохідПослКінцевСпож'
		then (case when isnull(v.[Glovo], 0)=isnull(v.[Рой], 0) or isnull(v.[Самовывоз shop.fora.ua], 0)=isnull(v.[Рой] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0)=0 then 0 
				else  isnull(v.[Рой], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0))*a.sum_vytraty end) end) 
	end [Рой],

case when name_vytraty in ('Бонус Е-СОМ', 'МаркетКурєрсДостав', 'НедоотриманнаВиручка') 
	then (case when isnull(v.[Glovo], 0)=isnull(v.[Нова пошта] , 0) then 0 
		else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)=0 then 0 
			else  isnull(v.[Нова пошта], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0))*a.sum_vytraty end) end) 
	when name_vytraty='ДохідПослКінцевСпож'
		then (case when isnull(v.[Glovo], 0)=isnull(v.[Нова пошта] , 0) or isnull(v.[Самовывоз shop.fora.ua], 0)=isnull(v.[Нова пошта] , 0) then 0 
			else (case when isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0)=0 then 0 
				else  isnull(v.[Нова пошта], 0)/(isnull(v.[ТО_всього], 0)-isnull(v.Glovo, 0)-isnull(v.[Самовывоз shop.fora.ua], 0))*a.sum_vytraty end) end) 
	end [Нова пошта]
...
when d.Розрахунок='НІ' then 0 
--when d.Розрахунок='Доставка' then a.sum_vytraty-t.[Нова пошта] 
when d.Розрахунок='100%' then bc.iPost 
when d.Розрахунок='FAGLL03' then -ff.[iPost] 
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[iPost %% в On-line]
...
when d.Розрахунок='НІ' then 0 
--when d.Розрахунок='Доставка' then a.sum_vytraty-t.[Нова пошта] 
when d.Розрахунок='100%' then bc.Uklon 
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[Uklon %% в On-line]
...
when d.Розрахунок='НІ' then 0 
when d.Розрахунок='100%' then bc.[Доставка shop.fora.ua] 
...
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[Доставка shop.fora.ua %% в On-line]
...
when d.Розрахунок='НІ' then 0 
--when d.Розрахунок='Доставка' then a.sum_vytraty-t.[Нова пошта] 
when d.Розрахунок='100%' then bc.[Самовывоз shop.fora.ua]
when d.Розрахунок='FAGLL03' then -ff.[Самовывоз shop.fora.ua] 
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[Самовывоз shop.fora.ua %% в On-line]
...
when d.Розрахунок='НІ' then 0 
--when d.Розрахунок='Доставка' then a.sum_vytraty-t.[Нова пошта]
when d.Розрахунок='100%' then bc.[Рой] 
when d.Розрахунок='FAGLL03' then -ff.[Рой] 
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[Рой %% в On-line]
...
when d.Розрахунок='НІ' then 0 
--when d.Розрахунок='Доставка' then a.sum_vytraty-t.[Нова пошта] 
when d.Розрахунок='100%' then bc.[Нова пошта] 
when d.Розрахунок='FAGLL03' then -ff.[Нова пошта] 
when d.Розрахунок='ФОТ' and a.type_vytraty not in (811000, 811001) 
	then (a.sum_vytraty-isnull(fop.sum_vytraty, 0))*[Доля On-line в ТО Магазину]*[Нова пошта %% в On-line]
...
group by [Міс.вин.п.]) ff on ff.[Міс.вин.п.]=a.mvz
	left join #bonuses_chan bc on a.mvz=bc.mvz and a.year=bc.year and a.month=bc.month and a.type_vytraty=bc.type_vytraty
  where a.month=@month and a.year=@year --and a.mvz='2021108200' --and [name_vytraty] in ('СобівРеалТоварВироб', 'СобівартРеалізТовар')


  /*select * from (select distinct mvz, [fil.Filials.filialNameUA] from  [Business_Analytic].[ecom].[TO_ecom] where year=2026 and month=1) t
  left join (select distinct mvz, filialName from [Business_Analytic].[ecom].[analyzer_mvz_final]  where year=2026 and month=1 and [Розрахунок]='Міша' 
  and [Меппінг]='Виручка') a on a.mvz=t.mvz
  where a.mvz is null*/
...
/*
  ...
  group by year, month, mvz, type, показник
	  ...
	  select sum(Всього) from [Business_Analytic].[ecom].[analyzer_mvz_final] a
	  where year=2026 and month=4 and [Признак_EBITDA]='Операційний потік'
